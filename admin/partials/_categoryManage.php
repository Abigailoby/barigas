<?php
    ...
    if(isset($_POST['createCategory'])) {
        ...
        $sql = "INSERT INTO `kategori` (`namaKategori`, `kategoriDesc`, `kategoriCreateDate`) VALUES ('$name', '$desc', current_timestamp())";   
    ...
    if(isset($_POST['removeCategory'])) {
        $catId = $_POST["catId"];
        $filename = $_SERVER['DOCUMENT_ROOT']."/barigas/image/kateg-".$catId.".jpg";
        $sql = "DELETE FROM `kategori` WHERE `kategoriId`='$catId'";   
        $result = mysqli_query($conn, $sql);
        if ($result){
    ...
    if(isset($_POST['updateCategory'])) {
        ...
        $sql = "UPDATE `kategori` SET `namaKategori`='$catName', `kategoriDesc`='$catDesc' WHERE `kategoriId`='$catId'";   
        $result = mysqli_query($conn, $sql);
        if ($result){
