...
if($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['addToCart'])) {
        ...
        $existSql = "SELECT * FROM `viewcart` WHERE barangId = '$itemId' AND `userId`='$userId'";
        ...
        if($numExistRows > 0){
            ...
            $sql = "INSERT INTO `viewcart` (`barangId`, `itemQuantity`, `userId`, `addedDate`) VALUES ('$itemId', '1', '$userId', current_timestamp())";   
    ...
    if(isset($_POST['removeItem'])) {
        $itemId = $_POST["itemId"];
        $sql = "DELETE FROM `viewcart` WHERE `barangId`='$itemId' AND `userId`='$userId'";   
        $result = mysqli_query($conn, $sql);
        ...
        if (password_verify($password, $passRow['password'])){ 
            ...
            if ($result){
                ...
                while($addrow = mysqli_fetch_assoc($addResult)){
                    $barangId = $addrow['barangId'];
                    $itemQuantity = $addrow['itemQuantity'];
                    $itemSql = "INSERT INTO `orderitems` (`orderId`, `barangId`, `itemQuantity`) VALUES ('$orderId', '$barangId', '$itemQuantity')";
            ...
            echo '<script>alert("KATA SANDI SALAH! Masukkan Kata Sandi Yang Benar.");
                    window.history.back(1);
                    </script>';
                    exit();
        }    
    ...
    if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest')
    {
        ...
        $qty = $_POST['quantity'];
        $updatesql = "UPDATE `viewcart` SET `itemQuantity`='$qty' WHERE `barangId`='$barangId' AND `userId`='$userId'";
